...
; implies that
;   x = y.
(assert (forall ((x $T$) (fc FUNCCONST)) (!
    (= x ($SortWrappers.$SnapTo$S$($SortWrappers.$S$To$Snap x fc)))
    :pattern (($SortWrappers.$S$To$Snap x fc))
    :qid |$Snap.$SnapTo$S$To$Snap|
    )))
(assert (forall ((x $Snap) (fc FUNCCONST)) (!
    (= x ($SortWrappers.$S$To$Snap($SortWrappers.$SnapTo$S$ x) fc))
