;
; ATTENTION:
; The first few lines can be uncommented when passing this file to Z3, but
; they must be commented when the file is used by Syxc.
;
; (set-option :auto-config false)
; (set-option :mbqi false)
; (declare-sort $S$)


(declare-fun $Seq.len ((List $S$)) Int)
; (define-const $Seq.nil (List $S$) nil)

; (declare-fun $Seq.elem (Int) Int)

; function Seq#Build<T>(s: Seq T, index: int, val: T, newLength: int) returns (Seq T);
(declare-fun $Seq.build ((List $S$) Int $S$ Int) (List $S$))

; ; function Seq#Append<T>(Seq T, Seq T) returns (Seq T);
; (declare-fun $Seq.con (Int Int) Int)
(declare-fun $Seq.con ((List $S$) (List $S$)) (List $S$))
; Assert defining equations for append as an axiom
(assert (forall ((xs (List $S$)) (ys (List $S$))) (!
  (ite (= nil xs)
       (= ($Seq.con xs ys) ys)
       (= ($Seq.con xs ys) (insert (head xs) ($Seq.con (tail xs) ys))))
  :pattern (($Seq.con xs ys))
  )))

; ; function Seq#Index<T>(Seq T, int) returns (T);
; (declare-fun $Seq.at (Int Int) Int)
(declare-fun $Seq.at ((List $S$) Int) $S$)
; Assert defining equations for at as an axiom
(assert (forall ((xs (List $S$)) (i Int)) (!
  (implies
    (and (<= 0 i) (< i ($Seq.len xs)))
    (ite (= i 0)
         (= ($Seq.at xs i) (head xs))
         (= ($Seq.at xs i) ($Seq.at (tail xs) (- i 1)))))
  :pattern (($Seq.at xs i))
  )))

; ; function Seq#Contains<T>(Seq T, T) returns (bool);
; (declare-fun $Seq.in (Int Int) Bool)
(declare-fun $Seq.in ((List $S$) $S$) Bool)
; Assert defining equations for in as an axiom
(assert (forall ((x $S$)) (!
  (not
    ($Seq.in nil x))
  :pattern (($Seq.in nil x))
  )))

(assert (forall ((xs (List $S$)) (x $S$)) (!
  (iff
    (not (= xs nil))
    (=
      ($Seq.in xs x)
      (or
        (= x (head xs))
        ($Seq.in (tail xs) x))))
  :pattern (($Seq.in xs x))
  )))

; ; function Seq#Equal<T>(Seq T, Seq T) returns (bool);
; (declare-fun $Seq.eq (Int Int) Bool)
(declare-fun $Seq.eq ((List $S$) (List $S$)) Bool)

(assert (forall ((xs (List $S$)) (ys (List $S$))) (!
	(iff
		($Seq.eq xs ys)
		(= xs ys))
	:pattern (($Seq.eq xs ys))
  )))

; ; function Seq#SameUntil<T>(Seq T, Seq T, int) returns (bool);
; (declare-fun $Seq.sameUntil (Int Int Int) Bool)

; ; function Seq#Take<T>(s: Seq T, howMany: int) returns (Seq T);
; (declare-fun $Seq.take (Int Int) Int)
(declare-fun $Seq.take ((List $S$) Int) (List $S$))

(assert (forall ((k Int)) (!
  (= ($Seq.take nil k) nil)
  :pattern (($Seq.take nil k))
  )))
  
(assert (forall ((xs (List $S$)) (k Int)) (!
  (ite (<= k 0)
    (= ($Seq.take xs k) nil)
    (=
      ($Seq.take xs k)
      (insert (head xs) ($Seq.take (tail xs) (- k 1)))))
  :pattern (($Seq.take xs k))
  )))
    

; ; function Seq#Drop<T>(s: Seq T, howMany: int) returns (Seq T);
; (declare-fun $Seq.drop (Int Int) Int)
(declare-fun $Seq.drop ((List $S$) Int) (List $S$))

(assert (forall ((k Int)) (!
  (= ($Seq.drop nil k) nil)
  :pattern (($Seq.drop nil k))
  )))
  
(assert (forall ((xs (List $S$)) (k Int)) (!
  (ite (<= k 0)
    (= ($Seq.drop xs k) xs)
    (=
      ($Seq.drop xs k)
      ($Seq.drop (tail xs) (- k 1))))
  :pattern (($Seq.drop xs k))
  )))

; ; function Seq#Range(min: int, max: int) returns (Seq int);
; (declare-fun $Seq.rng (Int Int) Int)

; ; axiom (forall<T> s: Seq T :: { Seq#Length(s) } 0 <= Seq#Length(s));
; (assert (forall ((xs Int)) (!
	; (<= 0 ($Seq.len xs))
	; :pattern (($Seq.len xs))
	; )))
; Assert defining equations for len as an axiom
(assert (forall ((xs (List $S$))) (!
  (ite (= nil xs)
       (= 0 ($Seq.len xs))
       (= (+ 1 ($Seq.len (tail xs))) ($Seq.len xs)))
  :pattern (($Seq.len xs))
  )))

; ; axiom (forall<T> :: Seq#Length(Seq#Empty(): Seq T) == 0);
; (assert (= ($Seq.len $Seq.nil) 0))

; ; axiom (forall<T> s: Seq T :: { Seq#Length(s) } Seq#Length(s) == 0 ==> s == Seq#Empty());
(assert (forall ((xs (List $S$))) (!
	(implies
		(= ($Seq.len xs) 0)
		(= xs nil))
	:pattern (($Seq.len xs))
  )))

; ; axiom (forall<T> t: T :: { Seq#Length(Seq#Singleton(t)) } Seq#Length(Seq#Singleton(t)) == 1);
; (assert (forall ((x Int)) (!
	; (= ($Seq.len ($Seq.elem x)) 1)
	; :pattern (($Seq.len ($Seq.elem x)))
	; )))


; ; axiom (forall<T> s: Seq T, i: int, v: T, len: int :: { Seq#Length(Seq#Build(s,i,v,len)) }
  ; ; 0 <= len ==> Seq#Length(Seq#Build(s,i,v,len)) == len);
(assert (forall ((xs (List $S$)) (i Int) (x $S$) (n Int)) (!
	(implies
		(<= 0 n)
		(= ($Seq.len ($Seq.build xs i x n)) n))
	:pattern (($Seq.len ($Seq.build xs i x n)))
	)))
	

; ; axiom (forall<T> s0: Seq T, s1: Seq T :: { Seq#Length(Seq#Append(s0,s1)) }
  ; ; Seq#Length(Seq#Append(s0,s1)) == Seq#Length(s0) + Seq#Length(s1));
(assert (forall ((xs (List $S$)) (ys (List $S$))) (!
	(=
		($Seq.len ($Seq.con xs ys))
		(+ ($Seq.len xs) ($Seq.len ys)))
	:pattern (($Seq.len ($Seq.con xs ys)))
	)))



; ; axiom (forall<T> t: T :: { Seq#Index(Seq#Singleton(t), 0) } Seq#Index(Seq#Singleton(t), 0) == t);
; (assert (forall ((x Int)) (!
	; (= ($Seq.at ($Seq.elem x) 0) x)
	; :pattern (($Seq.at ($Seq.elem x) 0))
	; )))

; ; axiom (forall<T> s0: Seq T, s1: Seq T, n: int :: { Seq#Index(Seq#Append(s0,s1), n) }
  ; ; (n < Seq#Length(s0) ==> Seq#Index(Seq#Append(s0,s1), n) == Seq#Index(s0, n)) &&
  ; ; (Seq#Length(s0) <= n ==> Seq#Index(Seq#Append(s0,s1), n) == Seq#Index(s1, n - Seq#Length(s0))));
(assert (forall ((xs (List $S$)) (ys (List $S$)) (i Int)) (!
	(and
		(implies
			(< i ($Seq.len xs))
			(=
				($Seq.at ($Seq.con xs ys) i)
				($Seq.at xs i)))
		(implies
			(<= ($Seq.len xs) i)
			(=
				($Seq.at ($Seq.con xs ys) i)
				($Seq.at ys (- i ($Seq.len xs))))))
	:pattern (($Seq.at ($Seq.con xs ys) i))
	)))
	
; ; axiom (forall<T> s: Seq T, i: int, v: T, len: int, n: int :: { Seq#Index(Seq#Build(s,i,v,len),n) }
  ; ; 0 <= n && n < len ==>
    ; ; (i == n ==> Seq#Index(Seq#Build(s,i,v,len),n) == v) &&
    ; ; (i != n ==> Seq#Index(Seq#Build(s,i,v,len),n) == Seq#Index(s,n)));
(assert (forall ((xs (List $S$)) (i Int) (x $S$) (n Int) (j Int)) (!
	(implies
		(and
			(<= 0 j)
			(< j n))
		(and
			(implies
				(= i j)
				(= ($Seq.at ($Seq.build xs i x n) j) x))
			(implies
				(not (= i j))
				(= ($Seq.at ($Seq.build xs i x n) j) ($Seq.at xs j)))))
	:pattern (($Seq.at ($Seq.build xs i x n) j))
	)))


; axiom (forall<T> s: Seq T, x: T :: { Seq#Contains(s,x) }
  ; Seq#Contains(s,x) <==>
    ; (exists i: int :: { Seq#Index(s,i) } 0 <= i && i < Seq#Length(s) && Seq#Index(s,i) == x));
(assert (forall ((xs (List $S$)) (x $S$)) (!
	(iff
		($Seq.in xs x)
		(exists ((i Int)) (!
			(and
				(<= 0 i)
				(< i ($Seq.len xs))
				(= ($Seq.at xs i) x))
			:pattern (($Seq.at xs i))
			)))
	:pattern (($Seq.in xs x))
	)))

; axiom (forall x: ref ::
  ; { Seq#Contains(Seq#Empty(), x) }
  ; !Seq#Contains(Seq#Empty(), x));
; (assert (forall ((x Int)) (!
	; (not ($Seq.in $Seq.nil x))
	; :pattern (($Seq.in $Seq.nil x))
	; )))
	
; ; axiom (forall<T> s0: Seq T, s1: Seq T, x: T ::
  ; ; { Seq#Contains(Seq#Append(s0, s1), x) }
  ; ; Seq#Contains(Seq#Append(s0, s1), x) <==>
    ; ; Seq#Contains(s0, x) || Seq#Contains(s1, x));
(assert (forall ((xs (List $S$)) (ys (List $S$)) (x $S$)) (!
	(iff
		($Seq.in ($Seq.con xs ys) x)
		(or
			($Seq.in xs x)
			($Seq.in ys x)))
	:pattern (($Seq.in ($Seq.con xs ys) x))
	)))
		
; ; axiom (forall<T> s: Seq T, i: int, v: T, len: int, x: T ::
  ; ; { Seq#Contains(Seq#Build(s, i, v, len), x) }
  ; ; Seq#Contains(Seq#Build(s, i, v, len), x) <==>
    ; ; (0 <= i && i < len && x == v)  ||  
    ; ; (exists j: int :: { Seq#Index(s,j) } 0 <= j && j < Seq#Length(s) && j < len && j!=i && Seq#Index(s,j) == x));
(assert (forall ((xs (List $S$)) (i Int) (x $S$) (n Int) (y $S$)) (!
	(iff
		($Seq.in ($Seq.build xs i x n) y)
		(or
			(and
				(<= 0 i)
				(< i n)
				(= y x))
			(exists ((j Int)) (!
				(and
					(<= 0 j)
					(< j ($Seq.len xs))
					(< j n)
					(not (= j i))
					(= ($Seq.at xs j) y))
				:pattern (($Seq.at xs j))
				))))
	:pattern (($Seq.in ($Seq.build xs i x n) y))
	)))
		
; ; axiom (forall<T> s: Seq T, n: int, x: T ::
  ; ; { Seq#Contains(Seq#Take(s, n), x) }
  ; ; Seq#Contains(Seq#Take(s, n), x) <==>
    ; ; (exists i: int :: { Seq#Index(s, i) }
      ; ; 0 <= i && i < n && i < Seq#Length(s) && Seq#Index(s, i) == x));
(assert (forall ((xs (List $S$)) (k Int) (x $S$)) (!
	(iff
		($Seq.in ($Seq.take xs k) x)
		(exists ((i Int)) (!
			(and
				(<= 0 i)
				(< i k)
				(< i ($Seq.len xs))
				(= ($Seq.at xs i) x))
			:pattern (($Seq.at xs i))
			)))
	:pattern (($Seq.in ($Seq.take xs k) x))
	)))
			
; ; axiom (forall<T> s: Seq T, n: int, x: T ::
  ; ; { Seq#Contains(Seq#Drop(s, n), x) }
  ; ; Seq#Contains(Seq#Drop(s, n), x) <==>
    ; ; (exists i: int :: { Seq#Index(s, i) }
      ; ; 0 <= n && n <= i && i < Seq#Length(s) && Seq#Index(s, i) == x));
(assert (forall ((xs (List $S$)) (k Int) (x $S$)) (!
	(iff
		($Seq.in ($Seq.drop xs k) x)
		(exists ((i Int)) (!
			(and
				(<= 0 k)
				(<= k i)
				(< i ($Seq.len xs))
				(= ($Seq.at xs i) x))
			:pattern (($Seq.at xs i))
			)))
	:pattern (($Seq.in ($Seq.drop xs k) x))
	)))


; ; axiom (forall<T> s0: Seq T, s1: Seq T :: { Seq#Equal(s0,s1) }
  ; ; Seq#Equal(s0,s1) <==>
    ; ; Seq#Length(s0) == Seq#Length(s1) &&
    ; ; (forall j: int :: { Seq#Index(s0,j) } { Seq#Index(s1,j) }
        ; ; 0 <= j && j < Seq#Length(s0) ==> Seq#Index(s0,j) == Seq#Index(s1,j)));
(assert (forall ((xs (List $S$)) (ys (List $S$))) (!
	(iff
		($Seq.eq xs ys)
		(and
			(= ($Seq.len xs) ($Seq.len ys))
			(forall ((i Int)) (!
				(implies
					(and
						(<= 0 i)
						(< i ($Seq.len xs)))
					(= ($Seq.at xs i) ($Seq.at ys i)))
				:pattern (($Seq.at xs i) ($Seq.at ys i))
				))))
	:pattern (($Seq.eq xs ys))
	)))
				
; ; axiom(forall<T> a: Seq T, b: Seq T :: { Seq#Equal(a,b) }  // extensionality axiom for sequences
  ; ; Seq#Equal(a,b) ==> a == b);
; (assert (forall ((xs Int) (ys Int)) (!
	; (implies
		; ($Seq.eq xs ys)
		; (= xs ys))
	; :pattern (($Seq.eq xs ys))
	; )))


; ; axiom (forall<T> s0: Seq T, s1: Seq T, n: int :: { Seq#SameUntil(s0,s1,n) }
  ; ; Seq#SameUntil(s0,s1,n) <==>
    ; ; (forall j: int :: { Seq#Index(s0,j) } { Seq#Index(s1,j) }
        ; ; 0 <= j && j < n ==> Seq#Index(s0,j) == Seq#Index(s1,j)));
; (assert (forall ((xs Int) (ys Int) (i Int)) (!
	; (iff
		; ($Seq.sameUntil xs ys i)
		; (forall ((j Int)) (!
			; (implies
				; (and
					; (<= 0 j)
					; (< j i))
				; (= ($Seq.at xs j) ($Seq.at ys j)))
			; :pattern (($Seq.at xs j) ($Seq.at ys j))
			; )))
	; :pattern (($Seq.sameUntil xs ys i))
	; )))
				

; ; axiom (forall<T> s: Seq T, n: int :: { Seq#Length(Seq#Take(s,n)) }
  ; ; 0 <= n ==>
    ; ; (n <= Seq#Length(s) ==> Seq#Length(Seq#Take(s,n)) == n) &&
    ; ; (Seq#Length(s) < n ==> Seq#Length(Seq#Take(s,n)) == Seq#Length(s)));
(assert (forall ((xs (List $S$)) (k Int)) (!
	(implies
		(<= 0 k)
		(and
			(implies
				(<= k ($Seq.len xs))
				(= ($Seq.len ($Seq.take xs k)) k))
			(implies
				(< ($Seq.len xs) k)
				(= ($Seq.len ($Seq.take xs k)) ($Seq.len xs)))))
	:pattern (($Seq.len ($Seq.take xs k)))
	)))
		
; ; axiom (forall<T> s: Seq T, n: int, j: int :: { Seq#Index(Seq#Take(s,n), j) } {:weight 25}
  ; ; 0 <= j && j < n && j < Seq#Length(s) ==>
    ; ; Seq#Index(Seq#Take(s,n), j) == Seq#Index(s, j));
(assert (forall ((xs (List $S$)) (k Int) (i Int)) (!
	(implies
		(and
			(<= 0 i)
			(< i k)
			(< i ($Seq.len xs)))
		(= ($Seq.at ($Seq.take xs k) i) ($Seq.at xs i)))
	:weight 25
	:pattern (($Seq.at ($Seq.take xs k) i))
	)))

; ; axiom (forall<T> s: Seq T, n: int :: { Seq#Length(Seq#Drop(s,n)) }
  ; ; 0 <= n ==>
    ; ; (n <= Seq#Length(s) ==> Seq#Length(Seq#Drop(s,n)) == Seq#Length(s) - n) &&
    ; ; (Seq#Length(s) < n ==> Seq#Length(Seq#Drop(s,n)) == 0));
(assert (forall ((xs (List $S$)) (k Int)) (!
	(implies
		(<= 0 k)
		(and
			(implies
				(<= k ($Seq.len xs))
				(= ($Seq.len ($Seq.drop xs k)) (- ($Seq.len xs) k)))
			(implies
				(< ($Seq.len xs) k)
				(= ($Seq.len ($Seq.drop xs k)) 0))))
	:pattern (($Seq.len ($Seq.drop xs k)))
	)))
		
; ; axiom (forall<T> s: Seq T, n: int, j: int :: { Seq#Index(Seq#Drop(s,n), j) } {:weight 25}
  ; ; 0 <= n && 0 <= j && j < Seq#Length(s)-n ==>
    ; ; Seq#Index(Seq#Drop(s,n), j) == Seq#Index(s, j+n));
(assert (forall ((xs (List $S$)) (k Int) (i Int)) (!
	(implies
		(and
			(<= 0 k)
			(<= 0 i)
			(< i (- ($Seq.len xs) k)))
		(= ($Seq.at ($Seq.drop xs k) i) ($Seq.at xs (+ i k))))
	:weight 25
	:pattern (($Seq.at ($Seq.drop xs k) i))
	)))

; ; axiom (forall<T> s, t: Seq T ::
  ; ; { Seq#Append(s, t) }
  ; ; Seq#Take(Seq#Append(s, t), Seq#Length(s)) == s &&
  ; ; Seq#Drop(Seq#Append(s, t), Seq#Length(s)) == t);
(assert (forall ((xs (List $S$)) (ys (List $S$))) (!
	(and
		(= ($Seq.take ($Seq.con xs ys) ($Seq.len xs)) xs)
		(= ($Seq.drop ($Seq.con xs ys) ($Seq.len xs)) ys))
	:pattern (($Seq.con xs ys))
	)))

;(push)
;(assert (not
;  (= ($Seq.len nil) 0)))
;(check-sat)
;(pop)
;
;(push)
;(declare-const xs (List $S$))
;(assert (not
;  (implies
;    (= ($Seq.len xs) 0)
;    (= xs (as nil (List $S$))))))
;(check-sat)
;(pop)
;
;(push)
;(declare-const xs (List $S$))
;(declare-const ys (List $S$))
;(assert (not
;  (=
;    ($Seq.len ($Seq.con xs ys))
;    (+ ($Seq.len xs) ($Seq.len ys)))))
;(check-sat)
;(pop)
;
;(push)
;(declare-const xs (List $S$))
;(declare-const ys (List $S$))
;(declare-const i Int)
;(assert (not
;	(and
;		(implies
;			(< i ($Seq.len xs))
;			(=
;				($Seq.at ($Seq.con xs ys) i)
;				($Seq.at xs i)))
;		(implies
;			(<= ($Seq.len xs) i)
;			(=
;				($Seq.at ($Seq.con xs ys) i)
;				($Seq.at ys (- i ($Seq.len xs))))))
;	))
;(check-sat)
;(pop)
;
;(push)
;(declare-const xs (List $S$))
;(declare-const x $S$)
;(declare-const i Int)
;(assert (not
;	(iff
;		($Seq.in xs x)
;		(exists ((i Int)) (!
;			(and
;				(<= 0 i)
;				(< i ($Seq.len xs))
;				(= ($Seq.at xs i) x))
;			:pattern (($Seq.at xs i))
;			)))
;	))
;(check-sat)
;(pop)
;
;(push)
;(declare-const x $S$)
;(assert ($Seq.in nil x ) )
;(check-sat)
;(pop)
;

; (push)
; (declare-const x $S$)
; (assert (not
  ; ($Seq.in
    ; (insert x nil)
    ; x)))
; (check-sat)
; (pop)

; (push)
; (declare-const x $S$)
; (assert (not
  ; (=
    ; ($Seq.at
      ; (insert x nil)
      ; 0)
    ; x)))
; (check-sat)
; (pop)

; (push)
; (declare-const x $S$)
; (assert (not
  ; ($Seq.in
    ; (insert x nil)
    ; x)))
; (check-sat)
; (pop)

; (check-sat)