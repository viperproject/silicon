...
class DependencyAwareChunkFactory(decider: Decider with DependencyAnalysisDeciderFeatures) extends ChunkFactory {
  ...
  def permMinus(chunk: GeneralChunk, newPerm: Term, newPermExp: Option[ast.Exp], analysisInfos: DependencyAnalysisInfos): GeneralChunk = {
    val newChunk = decider.registerDerivedChunk(Set(chunk), {finalPerm =>
      ...
      newPerm, analysisInfos.withDependencyType(DependencyType.Internal), isExhale=false, createLabel=false) // TODO ake: assumption type? maybe for exhale we want to have Implicit?
