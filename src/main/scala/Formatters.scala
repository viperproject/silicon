...
class DefaultStateFormatter[V, ST <: Store[V, ST], H <: Heap[H],
														S <: State[V, ST, H, S]]
			extends StateFormatter[V, ST, H, S, String] {
...
// """		
		...
		// var fs = π.flatMap(getFuncs).toSet.map((f: ast.Function) =>
	...
	// private def getVars(t: Term): List[Var] = t match {
		...
		// case sw: SortWrapper => getVars(sw.t0)
	...
	// private def getFuncs(t: Term): List[ast.Function] = t match {
		...
		// case sw: SortWrapper => getFuncs(sw.t0)
