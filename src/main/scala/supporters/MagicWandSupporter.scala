...
//              Q(h3, decider.fresh(sorts.Snap), Nil, c5))})})})})
    ...
    trait ActionWithHeuristics[I, O] {
      def c: C
      def initialInput: I
      def action(i: I, QS: O => VerificationResult, QF: Failure[ST, H, S] => VerificationResult): VerificationResult
      def heuristics()(implicit xxx: DummyImplicit): Seq[I => Either[Failure[ST, H, S], I]]
      def Q(o: O): VerificationResult
  }

    case class ConsumerActionWithHeuristics(σ: S, h: H, c: C)
                                               (val action: (S, H, C, (H, Term, List[CH], C) => VerificationResult, Failure[ST, H, S] => VerificationResult) => VerificationResult,
                                                val heuristics: Seq[(S, H, C) => Either[Failure[ST, H, S], (S, H, C)]],
                                                val Q: (H, Term, List[CH], C) => VerificationResult)
        extends ActionWithHeuristics[(S, H, C), (H, Term, List[CH], C)] {

      val initialInput = (σ, h, c)

      def action(inTuple: (S, H, C), QS: ((H, Term, List[CH], C)) => VerificationResult, QF: Failure[ST, H, S] => VerificationResult): VerificationResult =
        action(inTuple._1, inTuple._2, inTuple._3, scala.Function.untupled(QS), QF)
      
      def heuristics()(implicit xxx: DummyImplicit): Seq[((S, H, C)) => Either[Failure[ST, H, S], (S, H, C)]] =
        heuristics.map(_.tupled)
      
      def Q(outTuple: (H, Term, List[CH], C)) =
        Q.tupled(outTuple)
    }

    def tryWithHeuristic[I, O](awh: ActionWithHeuristics[I, O]): VerificationResult = {
      var currentInput = awh.initialInput
      var remainingReactions = awh.heuristics
        ...
        actionFailure match {
          ...
          case Some(_) =>
            ...
            while (   heuristicResult.isLeft
              ...
              && awh.c.program.fields.exists(_.name.equalsIgnoreCase("__CONFIG_HEURISTICS"))) {

              println(s"  applying next heuristic")
              heuristicResult = remainingReactions.head.apply(awh.initialInput)
