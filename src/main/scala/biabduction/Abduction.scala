        ...
        abductionUtils.permsTo(loc, q.s, q.v, q.lostAccesses) {
          ...
          case Some(permH: FractionalPerm) =>
            ...
            // pH < pG, reduce the goal and keep going
              ...
              Q(None)
              // This was moved into the AbdMissing rule when the Base rule was modified, left here for completeness
          ...
          // Happens for example with variables
          case None =>
            Q(None)
      ...
      case pred :: rest =>
        abductionUtils.checkChunk(pred, q.s, q.v, q.lostAccesses) {
          ...
          case Some(predChunk) =>
            ...
            // This is a bit more involved: if the trigger of our abduction is ``fold pred(x)`` then we do NOT want
            ...
            if (shouldSkip || shouldSkipInvariant || shouldSkipSamePred) {
              ...
              checkUnfoldingWand(q, pred) {
                ...
                case false =>
                  ...
                  // This is outdated, here for completeness, we now unfold all we have and check that we made progress
                  //
        ...
        evals(q.s, subexps, _ => pve, q.v)((s1, args, _, v1) => {
          ...
          matchingWand match {
            case Some(wand) =>
              ...
              // We can't apply a wand that has the same term on the LHS and RHS
              if (wand.left == wand.right) {
                Q(None)
              }
      ...
      case Some(wand) =>
          ...
          executionFlowController.locally(sReservedWand, q.v) { (s1, v1) =>
            ...
            case suc: NonFatalResult =>
              ...
              // We also need to filter out those statements that are a biproduct of assuming the LHS of the wand
              ...
              produces(sReservedWand, freshSnap, state.map(_._1) ++ assertions, _ => pve, q.v) { (s1, v1) =>
                ...
                abductionUtils.findChunks(locs, s1, v1, Internal()) { newChunks =>
                  ...
                  // Legacy code, here for completeness
