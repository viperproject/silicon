...
; XXXXXXXXXXXXXXX
; quantifiedpermissions\misc\unbounded.sil
; Locally evaluating above "forall x" entails evaluating x.f, which gives
; results in defining a fresh fvf. The corresponding definitions
; are nested under the outer "forall x", and not triggered when
; they are needed.
; XXXXXXXXXXXXXXX
...
(assert    (forall ((x@5 $Ref)) (!
      (=
        ($FVF.lookup_f fvf@6 x@5)
        ($FVF.lookup_f $t@4 x@5))
      :pattern (($FVF.lookup_f fvf@6 x@5))
      :pattern (($FVF.lookup_f $t@4 x@5))
    ))
)    
(assert    ($Set.eq ($FVF.domain_f fvf@6) ($FVF.domain_f $t@4)))
